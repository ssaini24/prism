...
async def github_webhook(
    ...
    total_cost = _calculate_pr_cost(diff_text)
    logger.info(
        "Posted review for PR #%d — %d issue(s) | estimated total cost: $%.6f",
        pr_number, total, total_cost,
    )
...
def _calculate_pr_cost(diff_text: str) -> float:
    """Estimate total LLM cost for this PR based on diff size and block count."""
    from core.diff_parser import parse_code_blocks, parse_diff
    from core.llm_client import _COST_PER_1K
    sql_blocks = parse_diff(diff_text)
    code_blocks = parse_code_blocks(diff_text) if settings.enable_code_review else []
    total_blocks = len(sql_blocks) + len(code_blocks)
    # Rough estimate: avg 500 input + 500 output tokens per block at haiku rates
    cost_in, cost_out = _COST_PER_1K.get("claude-haiku-4-5-20251001", (0.00025, 0.00125))
    return total_blocks * ((500 / 1000 * cost_in) + (500 / 1000 * cost_out))
